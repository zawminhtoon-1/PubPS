...
$LogMutexName  = "Global\MyPipeServer_Log"    # Named Mutex for thread-safe log writes
...
# ─────────────────────────────────────────────────────────────────────────────
# HELPER — Write timestamped entry to log file (mutex-protected)
# ─────────────────────────────────────────────────────────────────────────────
function Write-Log {
    param([string]$Message, [string]$Level = "INFO")
    $entry = "[{0}] [{1}] {2}" -f (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $Level, $Message
    $mtx = [System.Threading.Mutex]::new($false, $LogMutexName)
    try {
        [void]$mtx.WaitOne()
        [System.IO.File]::AppendAllText($LogFile, $entry + [System.Environment]::NewLine,
...
"@
    ...
    $ClientHandler = {
        ...
        # ── Mutex-protected log writer (shared across all runspaces) ──────────
        function Write-Log {
            param([string]$Message, [string]$Level = "INFO")
            $entry = "[{0}] [{1}] [C#{2}] {3}" -f `
                (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $Level, $ClientID, $Message
            $mtx = [System.Threading.Mutex]::new($false, $LogMutexName)
            try {
                [void]$mtx.WaitOne()
                [System.IO.File]::AppendAllText($LogFile, $entry + [System.Environment]::NewLine,
...
"@
    ...
    $clientCounter = 0L   # incremented only in the single-threaded accept loop
    ...
    try {
        while ($true) {
            ...
            # ── Wait for a free pipe slot before creating a new instance ──────
            # If all MaxRunspaces instances are busy, poll until one finishes
            # or the stop event fires.
            while ($ActiveJobs.Count -ge $MaxRunspaces) {

                # Non-blocking stop event check
                if ($stopEvent.WaitOne(0)) {
                    Write-Log "停止シグナルを受信しました（スロット待機中）。シャットダウンします。"
                    break
                }

                # Purge any completed jobs to free up slots
                $done = $ActiveJobs | Where-Object { $_.AsyncResult.IsCompleted }
                foreach ($job in $done) {
                    try   { $job.PS.EndInvoke($job.AsyncResult) } catch {}
                    $job.PS.Dispose()
                    $ActiveJobs.Remove($job)
                    Write-Log "スロットが解放されました。アクティブセッション数: $($ActiveJobs.Count)/$MaxRunspaces"
                }

                # Still full — wait a short interval before retrying
                if ($ActiveJobs.Count -ge $MaxRunspaces) {
                    Start-Sleep -Milliseconds 200
                }
            }

            # Re-check stop event after exiting the slot-wait loop
            if ($stopEvent.WaitOne(0)) {
            ...
            Write-Log "クライアント接続を待機中... (アクティブ: $($ActiveJobs.Count)/$MaxRunspaces)"
            ...
            # Increment counter — safe because accept loop runs on a single thread
            $clientCounter++
            $ClientID = $clientCounter
            Write-Log "クライアント #$ClientID を受け付けました — runspaceに割り当て中。(アクティブ: $($ActiveJobs.Count + 1)/$MaxRunspaces)"
            ...
            [void]$PS.AddParameter("LogMutexName",  $LogMutexName)
            ...
            # Purge any already-completed jobs
            $done = $ActiveJobs | Where-Object { $_.AsyncResult.IsCompleted }
            foreach ($job in $done) {
                try   { $job.PS.EndInvoke($job.AsyncResult) } catch {}
                $job.PS.Dispose()
