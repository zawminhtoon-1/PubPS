...
function Invoke-Stop {
    ...
    Write-Host "[サーバー] サーバーを停止しています (PID: $($state.PID))..." -ForegroundColor Cyan

    try {
        Stop-Process -Id $state.PID -Force -ErrorAction Stop
        Start-Sleep -Milliseconds 600
        Remove-Item $StateFile -Force -ErrorAction SilentlyContinue
        Write-Log "オペレーターによりサーバーが停止されました。" "WARN"
        Write-Host "[サーバー] サーバーを正常に停止しました。" -ForegroundColor Green
    }
    catch {
        Write-Host "[サーバー] 停止中にエラーが発生しました: $_" -ForegroundColor Red
    }
}
